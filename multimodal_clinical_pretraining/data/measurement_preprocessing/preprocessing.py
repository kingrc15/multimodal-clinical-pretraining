...
class Discretizer:
    ...
    def triplet_transform(self, X, header=None):
        if header is None:
            header = self._header
        ...
        for row in X:
            ...
            for j in range(1, len(row)):
                ...
                channel = header[j]
                f = self._possible_values[channel].index(row[j])
                v = row[j]

                data.append(np.array([t, f, v]))
